  ...
  useEffect(() => {
    if (!editMode && watchedTruckId && watchedTruckId.trim() !== "") {
      ...
      const autoCalculateDeadhead = async () => {
        try {
          const response = await fetch(`/api/trucks/${watchedTruckId}/last-location`);
          if (response.ok) {
            const lastLocation = await response.json();
            
            // Only populate deadhead fields if there's actual delivery history AND it's not a placeholder value
            if (lastLocation && lastLocation.city && lastLocation.city.trim() !== "") {
              ...
              if (originCity && originState) {
                ...
                if (deadheadMiles > 0) {
                  ...
                  toast({
                    ...
                    description: `${deadheadMiles} miles from truck's last location (${lastLocation.city}, ${lastLocation.state}) to pickup`,
                  });
                }
              }
            } else {
            ...
            // No delivery history found - clear deadhead fields completely
            console.log("No delivery history found - clearing deadhead fields");
            form.setValue("deadheadFromCity", "");
            form.setValue("deadheadFromState", "");
            form.setValue("deadheadMiles", 0);
