...
export default function TruckDemo() {
  ...
  const addTruckMutation = useMutation({
    mutationFn: async (truckData: any) => {
      ...
      console.log('[TruckDemo] Response status:', response.status, response.statusText);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error('[TruckDemo] Error response:', errorText);
        throw new Error(`HTTP ${response.status}: ${errorText || response.statusText}`);
      }
      
      const result = await response.json();
      console.log('[TruckDemo] Successfully parsed result:', result);
      return result;
    },
    onSuccess: (data) => {
