...
import { CheckCircle, AlertCircle, Truck, Activity, Loader2 } from 'lucide-react';
...
export default function IntegrationOnboarding() {
  ...
  const { data: providers } = useDemoQuery(
  ...
  const setupMutation = useMutation({
    mutationFn: async (data: IntegrationSetupData): Promise<SetupResult> => {
      ...
      const response = await fetch('/api/integration/setup', {
        method: 'POST',
                      ...
                      <div className="space-y-4 ml-6">
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <SelectContent className="bg-slate-700 border-slate-600">
                              {providers?.loadBoard.map((provider) => (
                                <SelectItem key={provider.name} value={provider.name} className="text-white">
                                  {provider.displayName}
                                </SelectItem>
                              ))}
