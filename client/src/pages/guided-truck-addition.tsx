...
export default function GuidedTruckAddition() {
  ...
  const addTruckMutation = useMutation({
    mutationFn: async (truckData: any) => {
      ...
      // Handle server response structure: { success: true, truck: {...}, id: "..." }
      if (result.truck && result.truck.id) {
        return result.truck; // Return the truck object directly
      } else if (result.id) {
        return result; // Fallback for direct truck object
      }
      
      throw new Error("Invalid server response: missing truck data");
    },
    onSuccess: (data) => {
      toast({
        title: "Truck Added Successfully!",
        description: `${data.name} has been added to your fleet with complete cost breakdown.`,
      });
      ...
      // Small delay to show success message, then redirect to truck profile
      setTimeout(() => {
        setLocation(`/truck/${data.id}`);
      }, 1500);
    },
    onError: (error) => {
      console.error('[GuidedTruckAddition] ERROR HANDLER - Error:', error);
      console.error('[GuidedTruckAddition] Error message:', error?.message);
      console.error('[GuidedTruckAddition] Error type:', typeof error);
  ...
  const handleSubmit = () => {
    ...
    const driverPayWeekly = ((parseFloat(costInfo.driverPayPerMile) || 0) / 100) * 3000;

    const truckData = {
      ...truckInfo,
      ...integrationOptions,
      fixedCosts: costTotals.totalFixedCosts,
      variableCosts: costTotals.totalVariableCosts,
      totalMiles: 0,
      isActive: 1,
      costBreakdown: {
        ...
        milesThisWeek: 3000,
        totalFixedCosts: costTotals.totalFixedCosts,
        totalVariableCosts: costTotals.totalVariableCosts,
        totalWeeklyCosts: costTotals.totalWeeklyCosts,
        costPerMile: costTotals.costPerMile,
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex justify-between mt-8">
            ...
            {currentStep < totalSteps ? (
              ...
              <Button
                onClick={(e) => {
                  console.log('[GuidedTruckAddition] ====== BUTTON CLICK DETECTED ======');
                  console.log('[GuidedTruckAddition] Click event:', e);
                  console.log('[GuidedTruckAddition] Event type:', e.type);
                  console.log('[GuidedTruckAddition] Button disabled state:', addTruckMutation.isPending);
                  console.log('[GuidedTruckAddition] Truck info:', truckInfo);
                  console.log('[GuidedTruckAddition] Integration options:', integrationOptions);
                  console.log('[GuidedTruckAddition] About to call handleSubmit...');
                  e.preventDefault();
                  e.stopPropagation();
                  handleSubmit();
                  console.log('[GuidedTruckAddition] handleSubmit called from button click');
                  console.log('[GuidedTruckAddition] ==============================================');
                }}
