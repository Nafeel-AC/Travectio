-- Corrected script to clean up duplicates based on actual schema
...
-- ==============================================
-- 5. CLEAN UP LOAD_STOPS TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 6. CLEAN UP FUEL_PURCHASES TABLE DUPLICATES
-- ==============================================

-- Check for duplicate fuel purchases (same truck, same date, same amount)
SELECT 'fuel_purchases' as table_name, "truckId", "purchaseDate", "totalCost", COUNT(*) as duplicate_count
FROM public.fuel_purchases 
GROUP BY "truckId", "purchaseDate", "totalCost" 
HAVING COUNT(*) > 1;

-- Remove duplicate fuel purchases
WITH fuel_purchase_duplicates AS (
  SELECT id, "truckId", "purchaseDate", "totalCost", "createdAt",
    ROW_NUMBER() OVER (PARTITION BY "truckId", "purchaseDate", "totalCost" ORDER BY "createdAt" ASC) as rn
  FROM public.fuel_purchases
)
...
-- ==============================================
-- 7. CLEAN UP HOS_LOGS TABLE DUPLICATES
-- ==============================================

-- Check for duplicate HOS logs (same driver, same timestamp)
SELECT 'hos_logs' as table_name, "driverId", timestamp, COUNT(*) as duplicate_count
FROM public.hos_logs 
GROUP BY "driverId", timestamp 
HAVING COUNT(*) > 1;

-- Remove duplicate HOS logs
WITH hos_log_duplicates AS (
  SELECT id, "driverId", timestamp, "createdAt",
    ROW_NUMBER() OVER (PARTITION BY "driverId", timestamp ORDER BY "createdAt" ASC) as rn
  FROM public.hos_logs
)
...
-- ==============================================
-- 8. CLEAN UP LOAD_PLANS TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 9. CLEAN UP LOAD_PLAN_LEGS TABLE DUPLICATES
-- ==============================================

-- Check for duplicate load plan legs (same load plan, same sequence)
SELECT 'load_plan_legs' as table_name, "planId", sequence, COUNT(*) as duplicate_count
FROM public.load_plan_legs 
GROUP BY "planId", sequence 
HAVING COUNT(*) > 1;

-- Remove duplicate load plan legs
WITH load_plan_leg_duplicates AS (
  SELECT id, "planId", sequence, "createdAt",
    ROW_NUMBER() OVER (PARTITION BY "planId", sequence ORDER BY "createdAt" ASC) as rn
  FROM public.load_plan_legs
)
...
-- ==============================================
-- 10. CLEAN UP LOAD_BOARD TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 11. CLEAN UP TRUCK_COST_BREAKDOWN TABLE DUPLICATES
-- ==============================================

-- Check for duplicate truck cost breakdowns (same truck, same week)
SELECT 'truck_cost_breakdown' as table_name, "truckId", "weekStarting", COUNT(*) as duplicate_count
FROM public.truck_cost_breakdown 
GROUP BY "truckId", "weekStarting" 
HAVING COUNT(*) > 1;

-- Remove duplicate truck cost breakdowns
WITH truck_cost_duplicates AS (
  SELECT id, "truckId", "weekStarting", "createdAt",
    ROW_NUMBER() OVER (PARTITION BY "truckId", "weekStarting" ORDER BY "createdAt" ASC) as rn
  FROM public.truck_cost_breakdown
)
...
-- ==============================================
-- 12. CLEAN UP FLEET_METRICS TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 13. CLEAN UP USER_ANALYTICS TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 14. CLEAN UP ACTIVITIES TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 15. CLEAN UP DATA_INPUT_TRACKING TABLE DUPLICATES
-- ==============================================

-- Check for duplicate data input tracking
SELECT 'data_input_tracking' as table_name, "userId", "inputType", "entityType", "inputDate"::date, COUNT(*) as duplicate_count
FROM public.data_input_tracking 
GROUP BY "userId", "inputType", "entityType", "inputDate"::date 
HAVING COUNT(*) > 1;

-- Remove duplicate data input tracking
WITH data_input_duplicates AS (
  SELECT id, "userId", "inputType", "entityType", "inputDate"::date, "createdAt",
    ROW_NUMBER() OVER (PARTITION BY "userId", "inputType", "entityType", "inputDate"::date ORDER BY "createdAt" ASC) as rn
  FROM public.data_input_tracking
)
...
-- ==============================================
-- 16. CLEAN UP SUBSCRIPTIONS TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 17. CLEAN UP PAYMENTS TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 18. CLEAN UP SESSIONS TABLE DUPLICATES
-- ==============================================
...
-- ==============================================
-- 19. CLEAN UP SESSION_AUDIT_LOGS TABLE DUPLICATES
-- ==============================================
